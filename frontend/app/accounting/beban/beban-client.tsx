...
export default function BebanClient() {
  ...
  async function persistJournal(status: 'draft' | 'posted') {
    ...
    try {
      const payload = buildJournalPayload(
        { ...generatedPreview, rows: mappedRows },
        selectedPeriodId,
        status,
      )
      await apiRequest('/api/journals/', { method: 'POST', body: payload })
      setSaveMessage(status === 'draft' ? 'Draft jurnal beban berhasil disimpan.' : 'Jurnal beban berhasil diposting.')
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <TransactionForm
          title="Form Beban"
          description="Akun diambil dari database apps_account. Gunakan pola: narasi nominal @Akun Kredit ke @Akun Debit untuk beban."
          placeholder="pak bambang membayar listrik 150.000 @Kas Umum ke @Beban Listrik"
