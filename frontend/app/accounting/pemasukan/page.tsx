...
function buildJournalCode(date: string) {
  ...
  return `INC-${dateToken}-${randomToken}`
}
...
export default function PemasukanPage() {
  ...
  async function persistJournal(status: 'draft' | 'posted') {
    ...
    try {
      ...
      setSaveMessage(status === 'draft' ? 'Draft jurnal pemasukan berhasil disimpan.' : 'Jurnal pemasukan berhasil diposting.')
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={`Catat penerimaan dana dengan prompt @akun, lalu tinjau jurnal hasil generate sebelum disimpan. ${selectedPeriod ? `Period terpilih: ${selectedPeriod.nama}.` : ''}`}
        actions={
          <>
            <Button
              variant="outline"
      ...
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <TransactionForm
          ...
          description="Akun diambil dari database apps_account. Gunakan pola: narasi nominal @Akun Kredit, lalu @Akun Debit."
          placeholder="Pak Bambang menyumbang uang sebesar 100.000 @Infaq, dimasukan ke @Kas Umum"
        ...
        />
          <JournalTable
            ...
            description="Preview jurnal umum ini dihasilkan dari parser prompt dan bisa diedit sebelum disimpan ke backend."
            rows={rows}
